...
from analyses.common.channel_specs import DistanceSpec, build_distance_channel, distance_spec_schema
from analyses.common.histogram import HistogramND
from analyses.common.reference_channels import radial_shell_volumes
...
class RDF(BaseAnalysis):
    ...
    CONFIG_SCHEMA = [
        ...
        distance_spec_schema(name="axis", label="the RDF distance channel"),
    ]
    ...
    def configure(self, config: RDFConfig):
        ...
        (
            ...
            self.obs_selection,
            self.channel,
        ) = build_distance_channel(self, self.ref_type, self.ref_key, self.axis, output_name="r/Angstrom")
        self.rebuild_runtime_state()
        self.n_ref = int(self.ref_indices.size)
        self.n_obs = int(self.obs_indices.size)
        self.hist = HistogramND([self.channel.bin_edges], "linear")
    ...
    def process_frame(self):
        ...
        for ref_molecule_index in range(batch.n_references):
            values = self.channel.values_for_reference(batch, ref_molecule_index)
            if values.size:
    ...
    def postprocess(self):
        ...
        fname = build_output_filename(
            "rdf",
            [
                format_selection(self.axis.ref_labels, self.ref_type.formula),
                format_selection(self.axis.obs_labels, self.obs_type.formula),
            ],
        )
