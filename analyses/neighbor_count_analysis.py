...
class ObservableConfig:
    """One collapsed neighbour-count observable around one reference site."""
...
class NeighborCountConfig:
    """Configuration for supported neighbour-count analysis."""
...
class NeighborCountAnalysis(BaseAnalysis):
    """Resolved neighbour-count analysis with one or more collapsed observables."""
                ...
                for observed_site, neighbors in per_site_neighbor_lists:
                    for neighbor_position in neighbors[ref_position]:
                        ...
                        if obs_atom_id == ref_atom_id:
                            continue
                        if observed_site.compound_key == self.ref_key:
                            if (
    ...
    def postprocess(self):
        if self.total_reference_sites == 0:
            console.warn("No neighbour-count values were accumulated.")
            return

        observable_parts = [self._format_observable_filename_part(observable) for observable in self.observables]
        joint_filename = build_output_filename("ncount_joint", observable_parts)
        joint_rows = [
        ...
        console.success(f"Saved neighbour-count results to {joint_filename}")
        ...
        for observable_index in range(len(self.observables)):
            ...
            marginal_filename = build_output_filename(
                f"ncount_obs{observable_index + 1}",
